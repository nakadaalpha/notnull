...
export default function Navbar() {
	...
	const isDarkHeroPage =
		location.pathname === "/" || location.pathname === "/warehouse";
	...
	return (
		<>
            ...
            {/* Mobile X button (inside) */}
						<button
							onClick={() => setIsMenuOpen(false)}
							className="absolute top-6 right-6 md:hidden w-10 h-10 rounded-full hover:bg-primary/10 flex items-center justify-center transition-all duration-300 hover:rotate-90"
						>
							<X size={20} strokeWidth={1.5} />
						</button>
						...
						<ul className="flex flex-col space-y-2">
							{menuItems.map((item, index) => (
								...
								>
									...
									>
										<span className="text-2xl font-light tracking-wide group-hover:translate-x-2 transition-transform duration-300">
											{item.name}
										</span>
										<ChevronRight
											size={20}
