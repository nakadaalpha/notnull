...
export default function CarDetail() {
		...
		if (car && car.brand) {
			try {
				const tracked = JSON.parse(
					localStorage.getItem("trackedBrands") || "[]",
				);
	...
	const handleReservationSubmit = async (e) => {
		...
		setSubmitMessage(
			"Generating Xendit Invoice... Redirecting to secure checkout...",
		);
	...
	if (loading) {
		return (
			<div className="min-h-screen pt-40 flex justify-center bg-background">
				<span className="text-sm font-light tracking-widest uppercase animate-pulse">
					Loading vehicle...
				</span>
			</div>
		);
	...
	if (!car) {
		return (
			<div className="min-h-screen pt-40 flex justify-center bg-background">
				<span className="text-sm font-light tracking-widest uppercase text-red-500">
					Vehicle not found.
				</span>
			</div>
		);
	...
	return (
		<div className="min-h-screen pt-32 pb-32 max-w-[1600px] mx-auto px-6 md:px-12 bg-background">
			<Link
				to="/warehouse"
				className="inline-flex items-center space-x-4 mb-10 group hover:opacity-60 transition-opacity"
			>
				<ArrowLeft
					size={20}
					strokeWidth={1}
				...
				<div className="w-full lg:w-1/3 flex flex-col lg:sticky lg:top-32 space-y-8">
					<div className="bg-secondary/5 dark:bg-white/5 border border-primary/10 rounded-xl p-8 md:p-10">
						<div className="mb-6 flex justify-between items-start">
							<span
								className={`px-4 py-1.5 border rounded-full text-[10px] font-bold uppercase tracking-[0.2em] ${
									car.stock > 0
							...
							{car.document &&
								car.document.has_bpkb &&
								car.document.has_stnk &&
								new Date(car.document.stnk_expiry_date) > new Date() && (
						...
						{specs?.hero_specs ? (
							<div className="flex flex-wrap gap-2 mb-8">
								{Object.entries(specs.hero_specs).map(([key, val]) => (
									<span
										key={key}
										className="px-3 py-1 bg-background border border-primary/10 rounded-md text-[10px] tracking-widest uppercase font-bold text-primary/70"
									>
										{val}
									</span>
								))}
							...
							<p className="text-sm text-primary/80 font-light mb-8 uppercase tracking-widest">
								Model Year {car.yearMade}
							</p>
						)}
					...
					<div className="flex flex-col md:flex-row justify-between items-end mb-12">
						<div>
							...
							<p className="text-primary/60 max-w-2xl text-lg">
								Based on your interest in the {car.brand?.name} {car.model}, we
								selected a few other extraordinary vehicles that match its
								caliber.
							</p>
						</div>
					</div>
