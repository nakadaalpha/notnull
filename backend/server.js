...
app.get('/api/run-seed', async (req, res) => {
  try {
    const porsche = await prisma.brand.upsert({ where: { name: 'Porsche' }, update: {}, create: { name: 'Porsche', imageUrl: '/images/brands/porsche.png' } });
    const bmw = await prisma.brand.upsert({ where: { name: 'BMW' }, update: {}, create: { name: 'BMW', imageUrl: '/images/brands/bmw.png' } });
    const mercedes = await prisma.brand.upsert({ where: { name: 'Mercedes-Benz' }, update: {}, create: { name: 'Mercedes-Benz', imageUrl: '/images/brands/mercedes.png' } });
    ...
    await prisma.user.upsert({
      ...
      update: {},
      create: { id: 'admin_001', username: 'admin', email: 'admin@notnull.com', password: hashedAdminPassword, role: 'ADMIN' }
    });
